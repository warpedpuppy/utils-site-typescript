...
function mountGlitter(target, options = {}) {
  ...
  return createLoop(runtime, (ctx, width, height, delta) => {
    ...
    rotation += 22e-5 * delta * (options.speed ?? 1);
...
function mountPrettyRing(target, options = {}) {
  const runtime = createRuntime(target, options);
  ...
  return createLoop(runtime, (ctx, width, height, delta) => {
    clock += delta * (options.speed ?? 1);
    rotation += 14e-5 * delta * (options.speed ?? 1);
    ctx.fillStyle = options.background ?? "#02050b";
    ctx.fillRect(0, 0, width, height);
    ctx.globalCompositeOperation = "lighter";
    const baseRadius = options.radius ?? Math.min(width, height) * 0.26;
    const pointerPulse = options.interactive && runtime.pointer.active ? Math.hypot(runtime.pointer.x - width / 2, runtime.pointer.y - height / 2) * 0.025 : 0;
    ctx.save();
...
function mountSparklies(target, options = {}) {
  const runtime = createRuntime(target, options);
...
function mountKlimt(target, options = {}) {
  const runtime = createRuntime(target, options);
  ...
  const stepRibbon = (ribbon) => {
    ...
    ribbon.x = next.x;
    ribbon.y = next.y;
    if (ribbon.curveCounter > ribbon.curveLimit || ribbon.x < -60 || ribbon.x > runtime.width + 60 || ribbon.y < -60 || ribbon.y > runtime.height + 60) {
...
function createLoop(runtime, frame, onResize) {
  let active = true;
  let paused = false;
  let rafId = 0;
...
function createRuntime(target, options) {
...
function attachRuntimeListeners(runtime, resize) {
  ...
  const observer = typeof ResizeObserver !== "undefined" ? new ResizeObserver(resize) : void 0;
...
function makeGlow(diameter, [r, g, b]) {
...
function fadeFill(background, alpha) {
...
function ribbonSeed(index, count, width, height) {
...
export { cosWave, mountGlitter, mountKlimt, mountPrettyRing, mountSparklies, nextKlimtBrickPoint, sparklyBeamPoint, tween };
